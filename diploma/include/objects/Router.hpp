...
class Router {
  public:
    ...
    void setHopTable(const t_RoutesTable &hopTable) {
      ...
      for (const auto &item : *m_hopTable) {
        kuznechik::t_Key key = m_key_pool[{item.second.encKeyId, item.second.keyType}]->getKey();
        ...
        std::cout << "[Router " << m_id << "] " << item.first << " -> "
                  << item.second.encKeyId << charFromKeyType(item.second.keyType) << " Key: " << keyStr << std::endl;
      }
    }
    ...
    bool connected(node::t_NodeId id) { return m_hopTable != nullptr && m_hopTable->find(id) != m_hopTable->end(); }
  ...
  private:
    ...
    std::map<std::pair<t_RouterId, t_RouterId>, std::pair<kuznechik::t_KeyId, KeyType>> m_key_use{};
